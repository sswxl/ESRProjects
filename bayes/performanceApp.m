classdef performanceApp < handle
    %
    % Usage:
    %     app.performanceApp
    ...
    properties (Constant)
        AppPathBase = {'/performance'};
        AppClass = 'performanceApp';
        Increment = 1;
        Decrement = 0;
        Version = '13a';
    end
    ...
    methods
        ...
        function startApp(obj)
            ...
            mlock;
            performanceApp.refcount(obj.Increment);
            ...
            existVal = exist(fullfile(pwd,'performance')); %#ok<EXIST>
            ...
            if (doesShadowExist && ~pathOne.equals(pathTwo))
                % We are trying to execute the wrong performance
                errordlg(message('MATLAB:apps:runapp:WrongEntryPoint', 'performance').getString, ...
            ...
            if nargout(@performance) == 0
                eval('performance');
            else
                obj.AppHandle = eval('performance');
            end
